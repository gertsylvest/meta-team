...
2. "Sprint": The project is delivered iteratively in sprints which have specific objectives, linked to the "project plan" milestones.
    ...
    - **Consult the prior sprint before planning the next**: sprint planning MUST begin by reading the previous sprint's result file and retrospective in full, plus the cumulative **Carryover Log** (`@/project-docs/sprint-outcomes/carryover-log.md`). Every recommendation, action item, and still-`open` carried-over item MUST be explicitly **Adopted**, **Deferred**, or **Dropped** — none may be silently ignored. Deferred items are recorded in the Carryover Log with a running count of how many times they have been deferred, so repeated slippage is visible at a glance rather than buried in per-sprint docs. **Any item that has been deferred 3 or more times MUST be escalated to the operator** for an explicit Adopt-or-Drop decision rather than being quietly carried over again. (The PM owns this mechanic — see the pm agent's Sprint Planning and Sprint End workflows.)
